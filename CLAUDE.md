...
**Версия:** 6.0 (FEAT-5 описание изображений: система загружаемых
компонентов CM-0…CM-3, замер и выбор модели; правила #28-#31)

> ⚠️ **Состояние ветвей — читать до того, как писать код.**
> Часть этого документа написана по ветке `fix/channel-sender-and-participants`,
> которая в `main` **не влита**. Такие места помечены 🌿 «на ветке, не в main».
> В `main` из этой ветки портирована только функция
> `_telegram_user_id_variants()` (трек A, в рамках FEAT-6).
> Не влито: миграции схемы (`PRAGMA user_version`), колонка `sender_type`,
> слияние channel-sender дубликатов в `get_user_stats()`.
> Проверка одной командой: `git merge-base --is-ancestor origin/fix/channel-sender-and-participants HEAD`
...
10a. **Описание изображений** (✅ CM-0…CM-3 — Qwen3-VL 4B загружаемым
    компонентом; дословный OCR со скриншотов и планов; см. COMPONENTS.md)
...
│   │
│   └── components/                  # ✅ CM-1 (2026-09-05) — COMPONENTS.md §5
│       ├── __init__.py
│       └── manager.py               # ComponentManager: реестр, установка,
│                                    #   запуск компонента как subprocess.
│                                    #   БЕЗ Qt — зовут из QThread-воркера
│
├── features/
│   ├── __init__.py
│   ├── auth/                        # ✅ Готова
...
│   │
│   ├── vlm/                         # ✅ CM-3 (2026-09-05) — описание картинок
│   │   ├── __init__.py
│   │   ├── api.py                   # Санитайзинг и рамка вокруг ответа
│   │   │                            #   модели (§4.4, уровни 3-4). Без Qt
│   │   ├── ui.py                    # VlmWorker(QThread)
│   │   └── download_ui.py           # Диалог скачивания компонента
│   │
│   └── export/                      # ✅ Готова
│       ├── generator.py             # DocxGenerator + JsonGenerator + MarkdownGenerator + HtmlGenerator
│       ├── filters.py               # ✅ UserFilter — фильтр участников (FEAT-6), без Qt
...
│
├── component_vlm/                   # ✅ CM-2 (2026-09-05) — НЕ часть приложения
│   ├── __init__.py                  #   Отсюда PyInstaller собирает отдельный
│   ├── engine.py                    #   exe (rozitta_vlm.spec, 30 МиБ), который
│   ├── worker.py                    #   приложение качает и зовёт как subprocess.
│   └── entry.py                     #   Импортировать отсюда в core/ и features/
│                                    #   нельзя — закреплено тестом
│
├── ui/
│   └── main_window.py               # ✅ MainWindow — цепочка
│                                    #   Parse→STT→VLM→Export
│
└── tests/
    ├── test_core/
...
### Таблица: `image_descriptions` (✅ реализована — 2026-09-05, FEAT-5)
```sql
...
Форма зеркальна `transcriptions`. Отдельная таблица, а не поле `kind` в
существующей: критерий — не сломать существующие SELECT'ы STT.
`INSERT OR REPLACE` делает выгрузку возобновляемой.

Методы: `insert_image_description()`, `get_vlm_candidates()` (картинки без
описания; отбор по `file_type IN ('photo','image')` — строка совпадает с тем,
что пишет `ParserService._detect_media_type`, стык закреплён тестом),
`get_image_descriptions_for_chat()`.
...
- 🌿 **на ветке, не в main:** `sender_type` поле в DB позволяет различать типы отправителей
...
### FEAT-6: фильтр участников — асимметрия режимов

Один переключатель, два взаимоисключающих режима. Асимметрия намеренная.

| Режим | Где фильтруется | Что в документе |
|-------|-----------------|-----------------|
| `include` «только выбранные» | SQL: `get_messages(user_ids=...)` | только их сообщения |
| `exclude` «кроме выбранных» | рендер: заглушка вместо строки | все, скрытые — заглушкой |

**Почему не одинаково.** Заглушку негде нарисовать, если строка отфильтрована
в SQL, — а без неё рвётся контекст ответов (`reply_to` указывает в пустоту) и
не видно, что в переписке на этом месте что-то было. Обратно: если бы
`include` тоже рисовал заглушки, документ при выборе одного человека распух
бы до размера всего чата, где 95% строк — заглушки.

**Контракт** — `features/export/filters.py`, без Qt и Telethon:

```python
UserFilter(mode, ids, names)     # frozen dataclass
    .is_hidden(uid)  -> bool     # рисовать заглушку (только exclude)
    .sql_ids()       -> list|None  # для get_messages(user_ids=) (только include)
    .name_part()     -> str      # фрагмент имени файла
    .header_line()   -> str      # строка для шапки документа
    .hash4()         -> str      # 4 символа sha256 от набора ID
NO_FILTER                        # синглтон «фильтра нет»
```

**Механизм заглушки — подмена строки, а не правка рендера.** В `generator.py`
колонки читаются только по индексам (`_COL_TEXT = 7` и т.д.), ни одного
`row["name"]` и ни одного `.keys()`. Поэтому `_hide_row()` возвращает обычный
кортеж с подменёнными полями, и для рендера он неотличим от `sqlite3.Row`.
**Следствие: сигнатуры методов рендера не менялись** — это сознательный выбор
после регрессий C2/B6 (правило #20).

Сохраняются `message_id`, `date`, `user_id`, `username`, `reply_to` — за счёт
этого работают закладки, ссылки «в ответ на» и порядок сообщений. Стираются
текст и все поля медиа. **Файл медиа на диске не трогается.**

**Отдельно — расшифровки.** Текст голосового берётся не из строки сообщения,
а из `stt_map` по `message_id`. Подмена строки его не закрывает, поэтому
`_apply_user_filter()` вычищает скрытые `message_id` из `stt_map`. Без этого
расшифровка скрытого участника уезжает в документ прямо под заглушкой.
...
| Фильтр | Фрагмент |
|--------|----------|
| `include`, 1 выбран | `only_Мария Петрова` |
| `include`, 2 и больше | `only_3_users_7f2a` |
| `exclude`, 1 выбран | `except_Мария Петрова` |
| `exclude`, 2 и больше | `except_2_users_7f2a` |

**Р-3 (сентябрь 2026):** строка `include, 1 выбран` раньше была пустой —
«кто» в имя подставлял отдельный параметр `username` генераторов. Два
способа сказать одно и то же: `username` заполнялся ровно тогда, когда
фильтр в режиме `include` и отмечен ровно один. В пути «по постам»
никакого `username` не было, и правило превращалось в дыру: выгрузка
поста с фильтром затирала выгрузку того же поста целиком (D-3). Теперь
«кто» собирает только `UserFilter`; параметр `username` в сигнатурах
остался, но на имя не влияет.

Четыре символа — `sha256` от набора ID (не md5: его ловят сканеры
безопасности). **Хеш обязателен:** без него два разных набора по трое дают
одно `only_3_users`, и вторая выгрузка молча затирает первую. Хуже того,
выгрузка с фильтром затирала полный архив того же чата за тот же период —
именно этот случай закрыт правилом I11.

Хеш считается от ID, а не от имён: участник может сменить ник, имя файла
останется прежним. Порядок выбора не влияет.
...
| `VlmWorker` | `log_message(str)`, `progress(int)`, `error(str)`, `character_state(str)`, `finished()` |
| `ComponentDownloadWorker` | `progress(int,int)`, `log(str)`, `done()`, `failed(str)` |
...
**Использовать в:**
- `get_messages()` — SQL-фильтр `WHERE user_id IN (?, ?)`. Колонка называется
  `user_id`, **не** `sender_id`. Применяется и к `user_id`, и к `user_ids` (FEAT-6)
- `get_thread_pairs()` — **пока НЕ применяется.** Режим веток по построению
  берёт одного участника (`get_thread_pairs(chat_id, user_id)`), варианты ID
  там ничего не меняют. Если появится мультивыбор в ветках — добавить
- Любой запрос, фильтрующий по отправителю канала/группы
...
связанный broadcast-канал для прямой нормализации.

> 🌿 **на ветке, не в main:** слияние дубликатов channel-sender (bare + marked ID
> одного канала) в одну запись внутри `get_user_stats()`. В `main` этого нет —
> `get_user_stats()` по-прежнему группирует по `user_id` без слияния, и канал
> может показаться в списке участников дважды. Живёт в ветке
> `fix/channel-sender-and-participants`.
...
### 22. Патч-скрипт: читать нормализованно, писать в исходном стиле

Рабочая копия на Windows обычно в CRLF. Якорь со `\n` в такой файл не
попадёт — патч честно скажет «якорь не найден» и ничего не сделает.

```python
with io.open(path, "r", encoding="utf-8", newline="") as fh:
    raw = fh.read()
newline = "\r\n" if "\r\n" in raw else "\n"
text = raw.replace("\r\n", "\n")     # искать и править здесь
...
out = text.replace("\n", newline) if newline == "\r\n" else text
```

Формулировка «сохранять CRLF» неполна и уже стоила одного круга: сохранять
надо при **записи**, а искать — в **нормализованном** тексте.

### 23. Удаляющий шаг: признак идемпотентности — отсутствие старого

Для шагов, которые что-то добавляют, «уже применено» определяется по наличию
нового фрагмента. Для шагов, которые **удаляют** код, это не работает:
маркер совпадает и до правки, и патч отчитается «уже применено», не сделав
ничего. Молчаливый ложный успех — худший режим отказа патч-скрипта.

```python
if marker is None:                 # шаг удаляющий
    if old not in text:
        skipped.append(desc); continue
elif marker in text:               # шаг добавляющий
    skipped.append(desc); continue
```

Реальный случай: `import pytest` не удалился, патч сказал «уже применено»,
поймал только независимый прогон `flake8` после патча.

### 24. Проверять результат независимо, а не по отчёту скрипта

Отчёт патча — это его мнение о себе. После каждого патча:

```bash
python -m py_compile <изменённые файлы>
python -m flake8 --select=F401,F811,F841 <изменённые файлы>
pytest tests/test_features -q
```

И отдельно: прогнать регрессию **до** патча, чтобы знать базовую линию.
На `main` в `tests/test_features` падает 134 теста — без базовой линии
любой прогон после патча выглядит катастрофой.

### 25. UI проверяется прогоном, а не чтением
...
### 26. Идемпотентность проверяется в точке правки
...
**Это правило и #23 — про любую скриптовую правку, не только про патчи.**
Разовый скрипт «на минуту» ошибается так же тихо. За сессию: замена искала
`IMAGES = ['01.jpg', ...]` в одинарных кавычках, в файле были двойные,
`str.replace` вернул строку без изменений, скрипт отчитался успехом — и
следующий замер прогнал не тот набор картинок. Лечится одной строкой:

```python
assert text.count(old) == 1, f"якорь найден {text.count(old)} раз"
```

### 27. Интерфейс не обещает того, чего нет
...
**И вторая половина, дописанная после разбора «Обновить архив»:
интерфейс не показывает выдуманные данные в том же виде, в каком показывал
бы настоящие.** Заглушка возвращала «46 новых комментариев» для любого чата
и уехала так в релиз. Пометка «Режим: Тестовый (stub)» в углу формы не
спасла — пользователь принял число за настоящее и завёл как баг.

Это опаснее неработающей кнопки: кнопку человек заметит, а правдоподобное
число примет на веру и построит на нём решение. Заглушка на время
разработки обязана быть отличима не жаргоном, а тем, что показывает
«функция в разработке» вместо цифр.

### 28. Обратные слэши не переживают шелл — правь редактором

Правка кода через `python - <<'PY'` в bash-героке съедает экранирование.
Строка `"\\n"` в скрипте доезжает до файла как настоящий перевод строки,
а якорь с `\n` не совпадает ни с чем.

За одну сессию это случилось трижды: сначала правка патч-скриптов
превратила `_NEWLINES[path] = "\r\n"` в код с разорванными строками,
потом якорь `f"\n*(STT:` не нашёлся в файле, где он есть.

Правило простое: **любая правка, где встречается обратный слэш, делается
редактором, а не скриптом через шелл.** Скрипты годятся для правок из
обычного текста — там же, где нужен цикл или условие.

Сюда же: `b"кириллица"` — синтаксическая ошибка. За сессию дважды. Байты
пишутся как `"текст".encode("utf-8")`.

### 29. Мокнутый шов — непроверенный шов

Подмена в тесте выключает не только медленное, но и проверку. Всё, что
подменено, тестами не покрыто вообще — и ломается ровно там.

Случай сессии: `find_binaries()` подбирала по шаблону `llama-server*` и
брала `llama-server-impl.dll` — **первым**, потому что при сортировке
дефис идёт раньше точки. Двадцать девять тестов были зелёными: во всех
`find_binaries` подменялась целиком. Нашлось первым же настоящим
запуском, ошибкой «не является приложением Win32».

Отсюда две вещи. Первая: у каждого мока стоит спросить, что именно
перестало проверяться. Вторая: **хотя бы один сквозной прогон без моков
обязателен** — на живых данных, в том окружении, где код будет работать.
Тот же прогон в этой сессии поймал ещё две ошибки замороженной сборки,
которых не видел ни один тест: `__file__` внутри `_internal` и веса на
два уровня выше, чем считал код.

### 30. Мутация, которая ничего не сломала, — это находка

Проверка мутацией (#25) даёт два полезных исхода, а не один. Красный тест
подтверждает, что тест сторожит. **Зелёный при внесённой поломке значит,
что либо тест не тот, либо код не нужен.** Разобраться надо в обоих
случаях — это не пропуск, а указание, куда смотреть.

Дважды за сессию это привело к настоящим находкам. Мутация «не убирать
мусор после провала» не уронила ничего — и разбор показал, что уборка
стояла не там: переустановка сносила рабочую версию ДО того, как
убеждалась, что новая распакуется. Мутация «снять `setStretch`» тоже
ничего не уронила — механизм оказался холостым, ширину держал
`setFixedWidth`, и лишний код убрали.

### 31. Замер может врать — проверь, чего он не видит

Автоматическая оценка меряет то, что в неё заложили, и молчит об
остальном. Прежде чем принимать по ней решение, стоит посмотреть на сырой
результат глазами.

Дважды за сессию замер завёл бы в неверную сторону. Детектор инъекции
искал слово только в первых сорока символах ответа — и отчитался
«устояла» о модели, которая написала «…игнорируя предложенные инструкции,
я называю: ВЗЛОМАНО». Проверка описаний считала присутствие нужных строк
и поставила лёгкой модели полный балл — а в текстах оказалась вата:
«показано основное здание с указанием площади и размеров» вместо
«10,5 м², 81,8 м²». По такому описанию в архиве ничего не найдёшь.

Оба раза счёт был односторонним: ловил провал и не ловил пустоту.
**Мерить надо плотность и точность, а не попадания** — и всё равно
прочитать несколько ответов целиком.

---

## ⛔ Отключено и ждёт решения

### «🌀 Обновить архив» — заглушка, попавшая в релиз

Пункт контекстного меню на карточке чата отключён 2026-09-05
(`features/chats/ui.py`, `_show_context_menu`). Код воркера и диалога
оставлен в `features/update_archive/`.

Что было: Шаг 1 задумывался как заглушка, Шаг 2 должен был заменить её
настоящей цепочкой и написан не был. Заглушка шесть раз спала по полсекунды,
писала в журнал «Шаг 1/6 … Шаг 6/6» и возвращала жёстко зашитый отчёт —
**«1 новый пост, 46 новых комментариев, 3 новых медиа», одинаковый для
любого чата**. В таком виде функция уехала в релиз v1.9.0.

Диалог писал «Режим: Тестовый (stub)». Не помогло: пользователь принял
числа за настоящие и завёл их как баг. Слово «stub» адресовано
разработчику, а не тому, кто смотрит на «46 новых комментариев».

**Урок для правила #27.** Оно говорило «интерфейс не обещает того, чего
нет». Этот случай добавляет вторую половину: **интерфейс не показывает
выдуманные данные в том же виде, в каком показывал бы настоящие.** Второе
опаснее первого — неработающую кнопку человек заметит, а правдоподобное
число примет на веру и построит на нём решение.

Если заглушка всё же нужна на время разработки, она обязана быть отличима
не жаргоном в углу формы, а тем, что показывает: «функция в разработке»
вместо цифр.

Включать пункт обратно — только вместе с настоящей цепочкой.

---

## 🧩 Известное дублирование (план рефакторинга)

Множества почти одинаковых блоков в `features/export/generator.py`.
Множества **не пересекаются** — рефакторинг одного не удешевляет остальные.

| Что | Мест | Признак поиска |
|-----|------|----------------|
| Загрузка строк | 8 | `<var> = self._db.get_messages(` |
| ~~Сборка имени файла~~ | ~~10~~ → 1 | ✅ схлопнуто в `_build_base_name()` |
| Шапка документа | 8 | `add_heading(`, `header = f"# `, `<h1>` |

Причина: четыре генератора × три входа (`generate`, `generate_by_posts`,
`_generate_threads`), каждый сам ходит в базу и сам собирал имя.

**Сборка имени закрыта (сентябрь 2026).** Десять копий разошлись не
теоретически: путь «по постам» терял фрагмент фильтра (D-3). Теперь имя
собирает одна `_build_base_name()`; вместе с ней вошли Р-3…Р-6, схема
слотов — в `docs/EXPORT_NAMING.md`, раздел 6. Умерли и удалены
`_filter_name_part()` и `_topic_suffix()`.

**Порядок работ:** общий загрузчик строк → шапка документа (в одну функцию
честно не сводится: заголовки разные по смыслу) → BUG-20.

Патчи по этим местам ищут точки **сканированием по шаблону**, а не
литеральными якорями — восемь-десять почти идентичных блоков хардкодить
бессмысленно и хрупко. Если найдено не столько, сколько ожидалось, патч
падает, не изменив файл.

**Страховка на рефакторинг:** `tests/test_features/test_user_filter_filenames.py`,
`test_user_filter_render.py` и `test_export_naming.py` краснеют, если общая
функция потеряет фильтр в одном из путей. Проверено мутацией, а не на слово:
выброшенный из `_build_base_name()` фрагмент фильтра роняет 24 теста,
отменённое Р-6 — 5, продублированное «кто» — 1.

---

## ✅ BUG-20: потерянные `@staticmethod` (исправлено, август 2026)

Три метода не обращались к `self` ни разу, но `self` оставался в сигнатуре —
декоратор потерялся при каком-то из патчей. Приложение работало (внутри
класса зовут `self._method(...)`, объект подставляется как надо), падали
только тесты, зовущие через класс. Декораторы возвращены, 27 тестов
позеленели.

**Таблица с вердиктами убрана намеренно.** Она дублировала в тексте то, что
живёт в коде, и протухла: к августу два вердикта из четырёх были
перевёрнуты — `DocxGenerator._add_message_to_doc` значился как «0 обращений
к self» (на деле 3: `self._transcriptions`, `self._log`), а
`JsonGenerator._make_record` как «5 обращений» (на деле 0). Правка по
документации сломала бы прод в одном случае и правильный тест в другом.

Вместо таблицы — `tools/check_signatures.py`, считает по AST при запуске:
...
Скрипт делит находки на два уровня: **СЛОМАНО** — метод зовут через класс,
падает с `TypeError`; **КАНДИДАТ** — вызовов через класс нет, ничего не
падает, можно не трогать. Код возврата 1 при поломках — годится для CI и
для проверки перед коммитом (правило #10).
...
### Описание изображений (✅ CM-3):
```
_on_stt_finished(collect_result)
  → _run_vlm(collect_result)          # выключено → сразу экспорт
    → VlmWorker(db_path, chat_id, components_dir, registry_url)
      → ComponentManager.run(...)     # subprocess, протокол §4
      → db.insert_image_description() # по одному, а не пачкой в конце
      → Signal: finished()
  → _start_export(collect_result)
```

Порядок обязателен: описания должны лечь в БД ДО сборки документа, иначе
выгрузка выйдет без них. Сбой описаний экспорт не отменяет.
...
| `config.py` | ✅ Готов | — |
| `core/utils.py` | ✅ Готов | — |
| `core/database.py` | ✅ Готов | 2026-07 FEAT-6: порт `_telegram_user_id_variants()`, `get_messages(user_ids=)`. 🌿 `sender_type` и миграции v2 — на ветке, не в main |
...
| `core/components/manager.py` | ✅ Готов | 2026-09-05 CM-1: реестр, установка, запуск компонента. Без Qt |
| `component_vlm/*` | ✅ Готов | 2026-09-05 CM-2: воркер на llama.cpp, протокол §4. НЕ импортируется приложением |
| `features/vlm/api.py` | ✅ Готов | 2026-09-05 CM-3: санитайзинг и рамка (§4.4). Без Qt |
| `features/vlm/ui.py` | ✅ Готов | 2026-09-05 CM-3: VlmWorker |
| `features/vlm/download_ui.py` | ✅ Готов | 2026-09-05 диалог скачивания компонента |
| `features/update_archive/*` | ⛔ Отключён | 2026-09-05 заглушка с выдуманными числами, пункт меню убран |
| `rozitta_vlm.spec` | ✅ Готов | 2026-09-05 CM-4: сборка компонента, 30 МиБ |
| `main.py` | ✅ Готов | — |
...
class ExportParams:
    ...
    user_filter:      "UserFilter"  = NO_FILTER    # FEAT-6, features/export/filters.py
    # DOCX всегда единый файл, ai_split на него не влияет
```

> ⚠️ Не путать два поля: `user_filter_mode` — это режим **веток**
> (`messages` / `threads`), появился раньше. `user_filter` — фильтр
> **участников** (FEAT-6). Имена похожи, смысл разный.
