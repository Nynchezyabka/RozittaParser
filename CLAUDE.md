...
## 🐞 BUG-20: потерянные `@staticmethod` (не исправлено)

Три метода не обращаются к `self` ни разу, но `self` остался в сигнатуре —
декоратор `@staticmethod` потерялся при каком-то из патчей:

| Метод | Обращений к `self` | Вердикт |
|-------|--------------------|---------|
| `MarkdownGenerator._format_message` | 0 | вернуть `@staticmethod`, убрать `self` |
| `HtmlGenerator._format_message` | 0 | то же |
| `DocxGenerator._add_message_to_doc` | 0 | то же |
| `JsonGenerator._make_record` | 5 | честный instance-метод, **неправ тест** |

Приложение работает: внутри класса зовут `self._format_message(row, stt)`, и
объект подставляется как надо. Падают только тесты, которые зовут через
класс — `test_export_md.py` (4) и `test_export_json.py` (3). Поэтому баг и
жил незаметно.

Чинить **после** рефакторинга общего загрузчика: раньше эти методы всё
равно будут переезжать.
