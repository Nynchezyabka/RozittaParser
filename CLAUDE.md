...
| Фильтр | Фрагмент |
|--------|----------|
| `include`, 1 выбран | *ничего* — имя подставит существующий `user_part` |
...
Три множества почти одинаковых блоков в `features/export/generator.py`.
...
| Сборка имени файла | 10 | `base_name = f"{safe_title}{topic_part}..."` (3 строки в строку) |
| Шапка документа | 8 | `add_heading(`, `header = f"# `, `<h1>` |

Причина: четыре генератора × три входа (`generate`, `generate_by_posts`,
`_generate_threads`), каждый сам ходит в базу и сам собирает имя.

**Порядок работ:** общий загрузчик строк + общая сборка имени (эти два
схлопнутся в одну точку каждый) → шапка документа (в одну функцию честно
не сводится: заголовки разные по смыслу) → BUG-20.
...
и `test_user_filter_render.py` краснеют, если общая функция потеряет фильтр
в одном из путей. Проверено: убранная вставка в `MarkdownGenerator` роняет
9 тестов из 14.
