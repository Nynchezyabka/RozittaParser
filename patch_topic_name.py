...
Безопасность:
    1. Создаются резервные копии (.bak) до любых изменений.
    2. Все замены применяются в памяти — файл не трогается до финальной записи.
    3. После записи проверяется синтаксис (py_compile).
    4. При любой ошибке на шагах 2-4 файл восстанавливается из .bak автоматически.
"""
...
class PatchError(Exception):
    """Ошибка применения патча — файл будет откачен."""
...
def apply_patches(path: Path, patches: list) -> None:
    """
    Применяет список замен к файлу атомарно.

    Алгоритм:
        1. Создать .bak
        2. Загрузить текст в память
        3. Применить все замены (только в памяти — файл не трогается)
        4. Записать результат во временный файл рядом с оригиналом
        5. Проверить синтаксис временного файла
        6. Переименовать временный → оригинал (атомарно на той же FS)
        7. При любой ошибке на шагах 3-6 восстановить из .bak
    """
    print(f"\n\U0001f4c2 {path}")
    bak = _backup(path)
...
GEN_PATCHES = [
    ...
    (
        "JsonGenerator _topic_suffix call",
...
print(f"""
{OK} \u041f\u0430\u0442\u0447 \u043f\u0440\u0438\u043c\u0435\u043d\u0451\u043d: 16 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u0439 \u0432 2 \u0444\u0430\u0439\u043b\u0430\u0445.

\u0427\u0442\u043e \u043e\u0441\u0442\u0430\u043b\u043e\u0441\u044c \u0441\u0434\u0435\u043b\u0430\u0442\u044c \u0432\u0440\u0443\u0447\u043d\u0443\u044e (ui/main_window.py):
