...
# ==============================================================================
# Загружаемые компоненты (COMPONENTS.md)
# ==============================================================================

class ComponentError(RozittaError):
    """
    Ошибка подсистемы загружаемых компонентов.

    Компонент — отдельная сборка, которую приложение качает по требованию
    и вызывает как subprocess (COMPONENTS.md). Всё, что может пойти не так
    на этом пути, наследуется отсюда: сеть, целостность архива, протокол,
    сам запуск.
    """


class RegistryError(ComponentError):
    """Не удалось получить или разобрать реестр компонентов."""


class ComponentIntegrityError(ComponentError):
    """
    Скачанный архив не совпал с объявленным sha256.

    Отдельный класс, а не общий ComponentError: причина почти всегда
    внешняя (битый прокси, оборванное зеркало), и лечится повтором,
    поэтому интерфейс предлагает именно «повторить», а не «сообщить
    об ошибке».
    """


class ComponentProtocolError(ComponentError):
    """
    Компонент говорит на протоколе, которого приложение не знает.

    Наступает при попытке запустить компонент новее самого приложения —
    единственный честный выход отсюда «обновите Rozitta Parser».
    """


class ComponentRunError(ComponentError):
    """Компонент запустился, но не отдал пригодного результата."""


class ComponentCancelled(ComponentError):
    """
    Операцию прервал пользователь.

    Это не сбой: наследуется от ComponentError только чтобы вызывающий мог
    поймать всё одним except, но обрабатывать её надо молча — без ошибки
    в журнале и без всплывающего сообщения.
    """
