...
- **BUG-20** — потерянные `@staticmethod` в трёх методах рендера + один
  неверный тест JSON. Подробно в CLAUDE.md. Чинить после рефакторинга
...
- **134 падающих теста на `main`** в `tests/test_features`. Часть похожа на
  окружение, но падения в `test_export_md.py` и `test_export_json.py`
  реальные — это BUG-20
