...
# Аддендум собирается динамически: пример цитирования берётся из реальной
...
def _first_post_example(exported_files: List[str]) -> Optional[Tuple[str, str]]:
    ...
    Нужен, чтобы пример цитирования ссылался на файл, который в этом
    архиве действительно есть.
    """
...
def _build_agent_addendum(
    ...
    Returns:
        Текст добавки, начинающийся с двух переводов строки.
        Пустая строка, если агент неизвестен.
    """
...
class KnowledgeBaseBuilder:
    ...
    def _build_instruction(
        ...
        _AGENT_ADDENDA: Dict[str, str] = {
            INSTRUCTION_CLAUDE: _build_agent_addendum(
                "claude", chat_meta, exported_files),
            INSTRUCTION_AGENTS: _build_agent_addendum(
                "codex", chat_meta, exported_files),
        }
        paths: List[str] = []
