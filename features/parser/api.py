...
class ParserService:
        ...
        while attempts <= _MAX_RETRIES:
            try:
                ...
                async for message in self._client.iter_messages(
                    ...
                    # Запоминаем посты с комментариями ДО проверки трекера:
                    # иначе в инкрементальном режиме уже скачанные посты
                    # отсекаются раньше регистрации и новые комментарии под
                    # старыми постами не докачиваются (хвост BF-2)
    ...
    async def _get_post_replies(
        ...
        BF-2: итерация идёт по КАНАЛУ с reply_to=post_id — Telethon шлёт
        GetRepliesRequest, и сервер возвращает ровно ветку комментариев
        поста (физически они лежат в linked группе, но нумерация ID у
        канала и группы независима, поэтому итерировать саму группу
        с reply_to=post_id нельзя — это и был баг BF-2).
        ...
        while attempts <= _MAX_RETRIES:
            try:
                # BF-2: итерируем по КАНАЛУ (GetRepliesRequest) — сервер
                # отдаёт только ветку комментариев поста, спам из linked
                # группы не попадает по определению протокола.
                ...
                # BF-2: у поста нет/удалено обсуждение — не ошибка, 0 комментариев
                self._log(f"    ℹ️ Пост #{post_id}: ветка обсуждения недоступна")
                logger.info("parser: post %s has no discussion thread", post_id)
                break
