...
class ChatItemWidget(QWidget):
    ...
    def _show_context_menu(self, pos) -> None:
        """
        Контекстное меню чата. Сейчас пустое — меню не показывается.

        ⛔ «🌀 Обновить архив» отключён (2026-09-05). Шаг 1 был заглушкой:
        воркер шесть раз спал по полсекунды, писал в журнал «Шаг 1/6 …
        Шаг 6/6» и возвращал жёстко зашитый отчёт — «1 новый пост,
        46 новых комментариев, 3 новых медиа» — одинаковый для любого чата.
        Шаг 2, который должен был заменить это настоящей цепочкой, не
        написан, а пункт меню уехал в релиз v1.9.0.

        Диалог отчёта писал «Режим: Тестовый (stub)», но это не спасло:
        пользователь принял выдуманные числа за настоящие и завёл их как
        баг. Слово «stub» — для разработчика, а не для того, кто смотрит
        на «46 новых комментариев».

        Правило #27 нарушалось тут дважды: интерфейс обещал функцию,
        которой нет, и показывал выдуманные числа в том же виде, в каком
        показывал бы настоящие. Второе хуже первого.

        Код воркера и диалога оставлен (features/update_archive/) —
        разбираться с ним отдельно. Включать пункт обратно можно только
        вместе с настоящей цепочкой обновления.
        """
        return
