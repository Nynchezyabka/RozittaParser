...
## Схема потока данных
Telegram API → ✅ AUTH → ✅ CHATS → ⚠️ PARSER (видео >50МБ) → ✅ DB → 🔴 EXPORT (нет дат) → 📤 Файлы
										↑
									параллельно:
...
1. **Smoke test** – зафиксировать текущие ошибки (30 мин)
2. **Исправить фильтрацию дат** – добавить `date_from/date_to` в `ExportParams` и `get_messages` (2–3 ч)
3. **Поправить скачивание больших видео** – адаптивный таймаут (1 ч)
4. **Решить STT в exe** – внешний venv или bundling (4–6 ч)
5. **Финальный тест** – все форматы + даты + .exe (1 ч)
