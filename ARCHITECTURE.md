...
|----------|-------|--------|-------|
| **Посты + комментарии** — не собираются корректно из-за спама в linked‑группе; требуется связывание пересланных постов с комментариями через `forward_from_message_id` | BF-2 | `parser`, `database` | `features/parser/api.py`, `core/database.py` |
